...
#define INF 1e9
#define BLOCKED 1e9 + 1
using namespace std;

void sendEdges(int rank, int size, int V, vector<vector<int>> &graph, vector<int> &isblocked){
    for(int i = 0 ; i < V ; i++){
        int proc = i % size;
        if(proc != rank){
            int num_neighbours = graph[i].size();
            MPI_Send(&isblocked[i], 1, MPI_INT, proc, 3*i,  MPI_COMM_WORLD);
            if(isblocked[i] == 0){
...
void receiveEdges(int rank, int size, vector<vector<int>> &neighbours, vector<int> &dist){
    int cnt = neighbours.size();
    int tag = rank;
    for(int i = 0 ; i < cnt ; i++){
        int isblocked;
        MPI_Status status;
        MPI_Recv(&isblocked, 1, MPI_INT, 0, 3*tag,  MPI_COMM_WORLD, &status);
        if(isblocked == 0){
            int num_neighbours;
...
int main(int argc, char* argv[]){
    ...
    // int len = isblocked.size();
    // MPI_Bcast(isblocked.data(), L, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if(rank == 0){
        ...
        sendEdges(rank, size, V, graph, isblocked);
        // cout << rank << endl;
        // for(int i = 0 ; i < nodes_cnt ; i++){
        //     cout << dist[i] << " ";
        // }
        ...
        neighbours.resize(nodes_cnt);
        receiveEdges(rank, size, neighbours, dist);
        // cout << rank << endl;
        // for(int i = 0 ; i < nodes_cnt ; i++){
        //     cout << dist[i] << " ";
        // }
    ...
    if(rank == 0){
        for(int i = 0 ; i < K ; i++){
            ...
            if(recvDistances[v_ind] == BLOCKED || recvDistances[v_ind] == INF){
                cout << -1 << " ";
            }
            else{
